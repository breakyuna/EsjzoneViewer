...
#### 本地会话恢复
应用会将登录成功后得到的 `ews_key` 和 `ews_token` 持久化到本地 Room 数据库，并在写入两个 Cookie 的同时保存当前站点。应用重新启动时会检查保存的会话：服务器明确返回未登录时才回到登录页；网络超时、断网、服务器错误或页面结构暂时无法识别时会保留本地会话，避免一次临时请求失败导致用户被迫重新登录。
