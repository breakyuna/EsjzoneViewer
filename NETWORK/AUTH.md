...
- 认证客户端还核对请求 host 与会话签发 host、当前账号身份及会话代次；切站、登录和登出使旧后台预同步任务失效。
- 登录流程先在临时 Cookie 容器中完成，只有登录响应成功且包含必要会话 Cookie 时才提交持久存储。
- 登录时用规范化邮箱在加密偏好中选取本地匿名书架身份；同一账号重新登录复用该身份。页面缓存命名空间仍可按登录轮换，登出仅清除当前身份指针，保留账号映射供下次登录恢复。旧版没有记录账号身份的书架行，在原会话键仍匹配时沿用；会话键变化时保留为待恢复书架，由用户在书架页确认归属后关联到当前账号并同步，不跨未知账号自动合并。
- 旧版 `domain:<host>` 书架行也保持待认领，不会自动分配给该域名的首个登录账号。
...
不持久化的校验/登出客户端仍使用传入的会话快照，但只对签发域名的 HTTPS 请求提供 Cookie。
