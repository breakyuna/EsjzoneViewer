...
- 登录页只显示一次账号输入，不提供站点选择。客户端按当前站点、另一站点的顺序分别执行完整登录，不保存密码；每站仅在网络异常时自动重试一次，认证拒绝不重试。任一站成功即可进入应用；若当前站失败而另一站成功，则将前台站点改为成功的站点。登录结果日志只记录站点和成功布尔值。切站后可访问共享本地数据和公开页面，界面明确提示“此站尚未登录”，用户可用同一账号补登；已知账号邮箱不匹配时拒绝补登。登录流程结束后无法在不保存密码或没有站点单点登录支持的情况下自动补登。不能假定一个域名签发的 Cookie 能在另一域名使用。
