...
- 登录页只显示一次账号输入，不提供站点选择。客户端按当前站点、另一站点的顺序分别执行完整登录，不保存密码；每站仅在 I/O 异常时自动重试一次，可解析的非成功业务状态不重试。任一站成功即可进入应用；若当前站失败而另一站成功，则将前台站点改为成功的站点。两站都失败时，只有均未返回明确的非网络结果才显示网络错误；服务端非成功状态、响应损坏或 Cookie 不完整显示通用登录失败，不推断为密码错误。登录结果日志只记录站点和成功布尔值。切站后可访问共享本地数据和公开页面，界面明确提示“此站尚未登录”，用户可用同一账号补登；已知账号邮箱不匹配时拒绝补登。登录流程结束后无法在不保存密码或没有站点单点登录支持的情况下自动补登。不能假定一个域名签发的 Cookie 能在另一域名使用。
...
- 收藏、资料和观看记录等账号页面的请求地址由该请求持有的授权站点决定；切换前已启动的后台任务不能用新设置域名拼接 URL。切站时重建各 Tab 的页面状态，避免书架模型继续使用旧站授权对象。
