...
1. 小型改动以静态检查作为日常验证；只有大规模改动后才执行本地 JVM 单元测试和 Android Lint。APK 构建由 GitHub Actions 验证。提交之前不用测试。
...
本地验证的目标是提前发现会使 GitHub Actions 中断的常见错误。Termux 已配置 Android SDK、JDK 和 Gradle。提交之前不用测试。小型改动（局部、影响范围明确的代码、资源或文档修改）只执行下述静态检查。大规模改动（跨多个模块或核心流程、涉及广泛重构等）完成后，执行与 CI 对应的 JVM 单元测试和 Android Lint：
