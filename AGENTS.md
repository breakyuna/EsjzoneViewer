...
1. 本项目任务中永远跳过 Gradle 构建、编译、测试、Lint 和依赖解析，不运行 `./gradlew`、Android Studio 构建任务或等价命令。
2. 验证只进行静态检查，包括代码阅读、引用搜索、`git diff --check`、配置文件格式检查和必要的轻量级脚本检查。
3. 不要为了验证而生成 `build/`、APK、AAB 或其他构建产物。
4. 不要在日志、异常信息、测试数据或文档中写入真实密码、会话 Cookie、`ews_key`、`ews_token` 或其他账户凭据。
5. 不要未经明确要求修改 `applicationId`、包名、数据库名称、远程仓库地址或发布签名配置。
6. 修改应保持最小范围，不要顺带重写无关代码或删除看似未使用但可能由资源、反射或网页解析流程依赖的内容。
...
2. 使用 `rg` 检查新增或删除的类名、资源名、路径和文档链接是否仍有有效引用。
3. 检查 Kotlin、XML、JSON、Markdown 文件是否存在明显语法错误或未闭合结构。
4. 检查新增用户可见文本是否同时存在英文和简体中文资源，或明确说明不需要本地化。
5. 检查敏感信息、构建产物和无关文件没有进入提交范围。

禁止以 Gradle 构建、测试、Lint 或 IDE 编译结果作为本项目任务的验证步骤。
